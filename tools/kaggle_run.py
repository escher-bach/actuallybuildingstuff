"""Thin STEP 2 control plane around the official Kaggle CLI."""
...
REGISTRY = ROOT / "step2" / "kaggle" / "experiments.toml"
...
def verify_remote_sha(remote: str, sha: str) -> None:
...
GIT_COMMIT = {sha!r}
CONFIG_REL = {config!r}
RUNTIME = Path("/tmp/step2-runtime")
SOURCE = RUNTIME / "baby-robot-foundations"
...
'''
    code3 = '''env["PYTHONPATH"] = str(SOURCE / "step2" / "python")
cmd = [
    sys.executable,
    "-m",
    "step2_experiments.runner",
        ...
        "cells": [
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": ["# STEP 2 two-T4 architecture/world vertical slice\n", "Generated launcher; repository code owns the experiment.\n"],
            },
            cell(code1),
            cell(code2),
            cell(code3),
...
def launch(name: str) -> str:
    ...
    verify_remote_sha(str(data["git_remote"]), sha)
    ...
    if dirty:
        ...
        print(dirty)
    with tempfile.TemporaryDirectory(prefix="step2-kaggle-") as temporary:
        staging = Path(temporary)
        notebook_name = "step2_t4x2_launcher.ipynb"
        (staging / notebook_name).write_text(
            json.dumps(
                notebook(str(data["repo_url"]), sha, config, str(data["output_root"])),
                indent=1,
            ),
            encoding="utf-8",
        )
    ...
    launch_dir = ROOT / "step2" / "audit" / "launches"
        ...
        json.dumps(
            {
                ...
                "git_remote_url": str(data["repo_url"]),
                "config": config,
...
def collect(kernel: str) -> Path:
    slug = kernel.split("/")[-1]
    launch_path = ROOT / "step2" / "audit" / "launches" / f"{slug}.json"
    ...
    pattern = (
        ...
        r"trivial-policy-baselines|capacity-probe|budget-curve|dagger-result|phase_status|environment|audit-manifest)\.json$"
        ...
        r"trivial-policy-baselines|gpu-vertical-slice|capacity-probe|budget-curve|dagger)\.log$"
    )
    command_run(
        [
            "kaggle",
    ...
    if args.command == "launch":
        ...
        command_run(
            [
                ...
                args.kernel,
                "--path",
                str(ROOT / "step2" / "audit" / "logs" / args.kernel.split("/")[-1]),
                "--force",
                "--file-pattern",
                r"(^|/)logs/.*\.log$",
            ]
