...
**Verified means read.** Repertoire Spec §2 says names and dates in it are leads to verify, not citations. A row with `verified = false` on every source is a lead no matter what else is filled in. Do not promote a row on the strength of a secondary description of it, and never invent a citation to fill a gap — an unverifiable row is worth less than an absent one because it looks like evidence.
